...
/**
 * Identifies scope annotations. A scope proxy applies to a class
 * containing an injectable constructor and governs how the injector reuses
 * instances of the type. By default, if no scope proxy is present, the
 * injector creates an instance (by injecting the type's constructor), uses
 * the instance for one injection, and then forgets it. If a scope proxy
 ...
 *
 * <p>In the following example, the scope proxy {@code @Singleton} ensures
 ...
 * proxy on the same class or a scope proxy it doesn't support.
 *
 * <p>A scope proxy:
 * <ul>
 ...
 * detect the case where a programmer used the scope proxy on a class but
